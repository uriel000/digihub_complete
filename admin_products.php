...
<html lang="en">
  ...
  <body>
    ...
    <h1 class="title"><span class="digihub">DigiHub</span> Shop products</h1>
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <select name="seller" class="box" required>
            <option disabled selected>Choose the seller</option>
            <?php
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                echo '<option disabled>No sellers found.</option>';
            }
            ?>
            </select>
    ...
    <section class="show-products">
        <div class="box-container">
            ...
            <div class="box">
                ...
                <a href="admin_products.php?delete=<?php echo $row['id'];?>" class="delete-btn" onclick="return confirm('Are you sure you want to delete this product?');">delete</a>
            </div>
            <?php
                    }
                }else{
    ...
    <section class="edit-product-form">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <!-- Update Seller -->
            <select name="update_seller" class="box" required>
            <option disabled selected>Choose the seller</option>
            <?php
            $currentSellerId = $fetch_update["seller_id"];
            $query = "SELECT * FROM users WHERE user_type = 'seller'";
            $result = mysqli_query($conn, $query);

            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    $selected = ($row['id'] == $currentSellerId) ? "selected" : "";
